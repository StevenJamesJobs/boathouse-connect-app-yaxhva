-- Seed the OpenTable Academy default host section for every new org at signup.
-- (Only change vs. prior version: the added PERFORM seed_org_host_sections line.)
CREATE OR REPLACE FUNCTION public.signup_owner_with_org(
  p_first_name text, p_last_name text, p_email text, p_password text,
  p_restaurant_name text, p_reward_currency_name text DEFAULT 'Bucks'::text,
  p_default_password text DEFAULT 'welcome123'::text, p_address text DEFAULT NULL::text,
  p_city text DEFAULT NULL::text, p_state text DEFAULT NULL::text,
  p_zip text DEFAULT NULL::text, p_weather_location text DEFAULT NULL::text)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
