...
export default function LibationRecipesEditorScreen() {
  ...
  const { categories: menuCats, refresh: refreshMenuCats } = useMenuCategories({ includeHidden: true, menuSlot: 1 });
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingRecipe) {
        ...
        const { error } = await supabase.rpc('update_libation_recipe', {
          p_user_id: user.id,
        ...
        await saveTranslations('libation_recipes', editingRecipe.id, { procedure_es: resolved.procedure.es }, user?.id);
        ...
        if (data) {
          await saveTranslations('libation_recipes', data as string, { procedure_es: resolved.procedure.es }, user?.id);
        }
  ...
  const screenTitle = organization?.menu_1_name ? `${organization.menu_1_name} ${t('libation_editor.title')}` : t('libation_editor.title');
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            actions={[
              {
                ...
                iosIcon: 'person.fill',
                androidIcon: 'person',
                onPress: () => router.replace('/libation-recipes'),
              },
              {
                key: 'cats',
      ...
      <MenuSearchRow
        ...
        placeholder={t('libation_editor.search_placeholder')}
        ...
        {Object.keys(recipesByCategory).length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={[styles.emptyText, { color: colors.textSecondary }]}>
              {searching ? t('cocktails.no_results') : t('libation_editor.no_recipes')}
            </Text>
          </View>
        ) : (
          <>
      ...
      <GlassActionSheet
        ...
        actions={actionsFor ? [
          ...
          {
            key: 'order',
            label: t('libation_editor.order_position'),
            iosIcon: 'list.number',
      ...
      <OrderPositionModal
        ...
        title={t('libation_editor.order_position')}
        subtitle={positionPicker ? t('libation_editor.order_position_subtitle', { name: positionPicker.recipe.name }) : undefined}
      ...
      <GlassSheet
        visible={showModal}
        onClose={closeModal}
        title={editingRecipe ? t('libation_editor.modal_edit_title') : t('libation_editor.modal_add_title')}
        footer={
          <View style={styles.footerRow}>
            <TouchableOpacity
              ...
              activeOpacity={0.8}
            >
              <Text style={[styles.footerBtnLabel, { color: colors.text }]}>{t('libation_editor.cancel_button')}</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[
                styles.footerBtn,
      ...
      >
        ...
        >
          ...
          <View style={styles.twoColRow}>
            ...
            <View style={styles.twoColRight}>
              <Text style={[styles.formLabel, { color: colors.textSecondary }]}>{t('libation_editor.price_label')}</Text>
              <TextInput
                style={[styles.formInput, { backgroundColor: colors.glass, color: colors.text, borderColor: colors.glassBorder }]}
                value={price}
        ...
        >
          ...
          <View style={styles.formField}>
            <Text style={[styles.formLabel, { color: colors.textSecondary }]}>{t('libation_editor.glassware_label')}</Text>
            <GlasswareIconPicker
              value={glassware}
          ...
          {/* Garnish */}
          <View style={styles.formField}>
            <Text style={[styles.formLabel, { color: colors.textSecondary }]}>{t('libation_editor.garnish_label')}</Text>
            <TextInput
              style={[styles.formInput, { backgroundColor: colors.glass, color: colors.text, borderColor: colors.glassBorder }]}
              value={garnish}
          ...
          {/* Ingredients */}
          <View style={styles.formField}>
            <Text style={[styles.formLabel, { color: colors.textSecondary }]}>{t('libation_editor.ingredients_label')}</Text>
        ...
        >
          {/* Procedure (auto-grow) */}
          <View style={styles.formField}>
            <Text style={[styles.formLabel, { color: colors.textSecondary }]}>{t('libation_editor.procedure_label')}</Text>
            <RichTextToolbar
        ...
        <SimpleSelectPicker
          ...
          options={[t('libation_recipes.featured'), ...cocktailSubOptions.map((o) => o.label)]}
          value={
            subcategoryId === FEATURED_SENTINEL
              ? t('libation_recipes.featured')
              : cocktailSubOptions.find((o) => o.id === subcategoryId)?.label || ''
          ...
          onSelect={(label) => {
            ...
            else if (label === t('libation_recipes.featured')) setSubcategoryId(FEATURED_SENTINEL);
          }}
