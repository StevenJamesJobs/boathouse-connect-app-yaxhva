...
export default function LibationRecipesEditorScreen() {
  ...
  // ··· meatball: Edit · Move up · Move down · Order Position · Delete (mirrors
  // menu-editor's openItemActions). In the horizontal list "up" = toward index 0.
  const openRecipeActions = (recipe: LibationRecipe, siblings: LibationRecipe[], index: number) => {
    const isFirst = index === 0;
    const isLast = index === siblings.length - 1;
    const editLabel = t('common.edit');
    const moveUpLabel = t('libation_editor.move_up');
    const moveDownLabel = t('libation_editor.move_down');
    const orderLabel = t('libation_editor.order_position');
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: colors.card }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>
          {organization?.menu_1_name ? `${organization.menu_1_name} ${t('libation_editor.title')}` : t('libation_editor.title')}
        </Text>
